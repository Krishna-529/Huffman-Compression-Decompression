...
const CompressionPanel = () => {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: false,
    accept: {
      "text/*": [".txt", ".csv", ".json", ".xml", ".html", ".css", ".js", ".md"],
    },
  });
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      const compressedFilename = `${file.name}.bin`;
  ...
  return (
    <Card className="border-border/50 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Upload a text file to compress it using Huffman encoding
        </CardDescription>
      </CardHeader>
        ...
        {isProcessing && (
          <div className="space-y-2">
            ...
            <p className="text-sm text-center text-muted-foreground">
              Running Huffman compression algorithm...
            </p>
          </div>
        )}
        ...
        {result && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
              ...
              <span className="font-medium">Compression Successful!</span>
            </div>
